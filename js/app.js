...
const App = {
  ...
  renderAnnuityPage() {
    ...
    var cx = 180, cy = 180, rOuter = 120, rInner = 72;
    var cumulative = 0;
    var slicesSvg = "";
    var legendHtml = "";
    ...
    var svgHtml = '<svg viewBox="0 0 360 360" class="annuity-donut">' +
      slicesSvg +
  ...
  _bindExtraTransactionButtons() {
    ...
    var countEl = document.getElementById('extraTransactionCount');

    function renderList() {
      var txns = self._retirementParams.extraTransactions || [];
      if (countEl) {
        countEl.textContent = txns.length > 0 ? '(' + txns.length + '条)' : '';
      }
      if (!listEl) return;
      if (txns.length === 0) {
